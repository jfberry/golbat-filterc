...
func TestParseErrors(t *testing.T) {
	cases := []struct{ src, want string }{
		...
		{" \n\t ", "1:1: empty expression"},
		{"not iv in [1]", "1:1: not binds tighter than in; write not (iv in [1])"},
		{"iv == 1 || ! iv in 1..3", "1:12: ! binds tighter than in; write ! (iv in 1..3)"},
		{"pokemon in [0, 1]", `1:13: pokemon 0 is not a species id; leave pokemon unconstrained for "everything else"`},
